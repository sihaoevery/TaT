...
class CustomLoss(nn.Module):
    def __init__(self, criterion_config,**kwargs):
        super().__init__()
        term_dict = dict()
        ...
        if sub_terms_config is not None:
            for loss_name, loss_config in sub_terms_config.items():
                ...
                sub_criterion = get_single_loss(sub_criterion_config, loss_config.get('params', None),**kwargs)
...
class GeneralizedCustomLoss(CustomLoss):
    def __init__(self, criterion_config,**kwargs):
        super().__init__(criterion_config,**kwargs)
...
def get_custom_loss(criterion_config,**kwargs):
    criterion_type = criterion_config['type']
    if criterion_type in CUSTOM_LOSS_CLASS_DICT:
        return CUSTOM_LOSS_CLASS_DICT[criterion_type](criterion_config,**kwargs)
