...
from torchdistill.feature_visualization import FeatureVisualizer
from einops import rearrange
from torch import einsum
from torchdistill.models.classification.vit import Attention, CONFIGS
from torchdistill.models.util import wrap_if_distributed
import os
...
class KDLoss(nn.KLDivLoss):
    ...
    def __init__(self, temperature, alpha=None, beta=None, reduction='batchmean', **kwargs):
        ...
        self.cross_entropy_loss = nn.CrossEntropyLoss(reduction=cel_reduction) #for compatible
        logger.info("Using KDLoss, alpha:{}, beta:{}".format(self.alpha,self.beta))
...
class ChSimLoss(nn.Module):
    ...
    def __init__(self, feature_pairs, **kwargs):
        super().__init__()
        self.feature_pairs = feature_pairs
        #self.vis_feator = FeatureVisualizer(save_dir = "/mnt_venus/luis.ll/venus/vis/0_0/")
    ...
    def batch_loss(self, f_s, f_t):
        #self.vis_feator(f_s,["student"],{'input':input},['input'])
        ...
        #emd_s = emd_s.mean(dim = 2)
        #emd_t = emd_t.mean(dim = 2)
        G_diff = emd_s - emd_t
...
class TransViT(nn.Module):
    """
        Transfer the attention of ViT to CNN
    """
    def __init__(self, feature_pairs, img_size, vit_type, vis, device, device_ids, distributed, **kwargs):
        super().__init__()
        self.feature_pairs = feature_pairs
        vit_config = CONFIGS[vit_type]
        patch_size = vit_config.patches["size"]
        self.n_patches = (img_size[0] // patch_size[0]) * (img_size[1] // patch_size[1]) 
        self.n_patches += 1 # positional embedding dimension
        self.hidden_size = vit_config.hidden_size

        # # self.embedding  = torch.nn.Linear(512*7*7, self.n_patches*self.hidden_size).cuda() # for resnet18
        # self.conv_1 = nn.Conv2d(512,9456,kernel_size=4,stride=1,padding=0,bias=False).cuda()
        # self.attention_norm = torch.nn.LayerNorm(vit_config.hidden_size, eps=1e-6).cuda()
        # self.attn = Attention(vit_config, vis).cuda()

        # self.conv_1 = wrap_if_distributed(nn.Conv2d(512,9456,kernel_size=4,stride=1,padding=0,bias=False),device=device,device_ids=device_ids,distributed=distributed)
        # self.attention_norm = wrap_if_distributed(torch.nn.LayerNorm(vit_config.hidden_size, eps=1e-6),device=device,device_ids=device_ids,distributed=distributed)
        # self.attn = wrap_if_distributed(Attention(vit_config, vis),device=device,device_ids=device_ids,distributed=distributed)
    ...
    def batch_loss(self, f_s, f_t):
        t_out, t_attn_weights= f_t

        # # print('Original f_s shape is',f_s.shape)
        # f_s = self.conv_1(f_s)
        # # print('After Conv f_s shape is',f_s.shape)
        # f_s = f_s.view(-1,self.n_patches,self.hidden_size)
        # # print('After view f_s shape is',f_s.shape)
        # f_s = self.attention_norm(f_s)
        # s_out, s_attn_weights=self.attn(f_s)
        # loss = 0.5*nn.MSELoss()(t_out,s_out) + 0.5*nn.MSELoss()(t_attn_weights,s_attn_weights)

        s_out, s_attn_weights = f_s
        loss = 0.5*nn.MSELoss()(t_out,s_out) + 0.5*nn.MSELoss()(t_attn_weights,s_attn_weights)
...
def get_single_loss(single_criterion_config, params_config=None,**kwargs):
    loss_type = single_criterion_config['type']
    single_loss = SINGLE_LOSS_CLASS_DICT[loss_type](**single_criterion_config['params'],**kwargs) \
        if loss_type in SINGLE_LOSS_CLASS_DICT else func_util.get_loss(loss_type, single_criterion_config['params'],**kwargs)
    if params_config is None:
        return single_loss
    return get_loss_wrapper(single_loss, params_config, params_config.get('wrapper', dict()),**kwargs)
