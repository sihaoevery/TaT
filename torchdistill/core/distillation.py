...
class DistillationBox(nn.Module):
    ...
    def setup_loss(self, train_config):
        ...
        self.criterion = get_custom_loss(criterion_config,device=self.device,
                        device_ids=self.device_ids, distributed=self.distributed)
        ...
        if len(optim_config) > 0:
            ...
            if len(module_wise_params_configs) > 0:
                ...
                if self.teacher_updatable:
                    ...
                    trainable_module_list.append(self.teacher_model.embed_dict)
            logger.info("*****Trainable List*****")
            logger.info(trainable_module_list)
    ...
    @torch.no_grad()
    def _momentum_update_teacher(self,m=0.0001):
        '''
        Experimental attempt for self-distillation. 
        Not used in this codeset.
        Update the params of teacher model with the momentum m
        This function follows the code from moco.
        '''
        for param_s, param_t in zip(self.student_model.model.parameters(), self.teacher_model.model.parameters()):
