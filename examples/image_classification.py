...
def seed_torch(seed=1029):
    ...
    # CAUTION! The options below ensure the reproducibility 
    # but VERY SLOW.
...
def get_argparser():
    ...
    parser.add_argument('-adjust_lr', action='store_false',
...
def train(teacher_model, student_model, dataset_dict, ckpt_file_path, device, device_ids, distributed, config, args):
    ...
    for epoch in range(args.start_epoch, training_box.num_epochs):
        ...
        if val_top1_accuracy > best_val_top1_accuracy and is_main_process():
            #Save best student model. Only the classification model.
        ...
        if  (epoch == 0 or (epoch+1)%20==0 ) and is_main_process():
            # regular save student checkpoint with the query&value linear function 
            trainbox_student_wo_ddp = training_box.student_model.module if module_util.check_if_wrapped(training_box.student_model) \
                                        else training_box.student_model
            ckpt_path = os.path.join('./model/','epoch_{}.pt'.format(epoch))
            logger.info('Regular saving checkpoint into {}'.format(ckpt_path))
            save_ckpt(trainbox_student_wo_ddp, optimizer, lr_scheduler,
                      val_top1_accuracy, config, args, ckpt_path)
...
def main(args):
    ...
    logger.info("Using fixed seed %d..."%lucky_number)
